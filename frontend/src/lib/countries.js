/**
 * ISO 3166-1 alpha-2 country list with dial codes.
 * Each entry: { code, name, flag, dialCode }
 */
...
export const COUNTRIES = [
  ...
  { code: 'VC', name: 'Saint Vincent & Grenadines',   flag: '🇻🇨', dialCode: '1784' },
...
/** Items for the country v-autocomplete: { title, value } */
export const COUNTRY_ITEMS = COUNTRIES.map(c => ({
  title: `${c.flag} ${c.name}`,
  value: c.code,
}));

/** Items for the dial-code v-autocomplete: { title, value } */
export const DIAL_CODE_ITEMS = COUNTRIES.map(c => ({
  title: `${c.flag} +${c.dialCode}`,
  subtitle: c.name,
  value: c.dialCode,
}));
